...
class Schedules extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('weekday', 'required'),
			array('weekday', 'numerical', 'integerOnly'=>true),
			array('starttime, stoptime', 'length', 'max'=>5),
			array('courseName', 'length', 'max'=>255),
			array('notes', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('scheduleid, weekday, starttime, stoptime, courseName, notes', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('scheduleid',$this->scheduleid,true);
